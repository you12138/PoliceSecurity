...
//
//  Created by 王化楠 on 2017/1/17.
//  Copyright © 2017年 whn. All rights reserved.
//

#import "KKColumnChart.h"

@interface KKColumnChart ()<CAAnimationDelegate>
...
-(UIScrollView *)BGScrollView{
    ...
    if (!_BGScrollView) {
        ...
        self.bgVewBackgoundColor = _bgVewBackgoundColor;
...
-(void)setLineValueArray:(NSArray *)lineValueArray{
    ...
    if (max<5.0) {
        ...
        _maxHeight = max + max/5;
    }
    
    _maxHeight += 4;
    _perHeight = (CGRectGetWidth(self.frame) - 30 - _originSize.x)/_maxHeight;
...
-(void)showAnimation{
    ...
    if (_needXandYLine) {
        ...
        [bezier addLineToPoint:P_M(CGRectGetWidth(self.frame) - 2*self.originSize.x, CGRectGetHeight(self.frame) - self.originSize.y)];
        ...
        for (NSInteger i = 0; i<5; i++) {
            ...
            [second moveToPoint:P_M(_originSize.x + height, CGRectGetHeight(self.frame) - _originSize.y)];
            [second addLineToPoint:P_M(_originSize.x + height, 20)];
            ...
            textLayer.frame = CGRectMake(_originSize.x + height - be/2, CGRectGetHeight(self.frame) - _originSize.y+5, be, 15);
    ...
    if (_xShowInfoText.count == _valueArr.count&&_xShowInfoText.count>0) {
        ...
        for (NSInteger i = 0; i<_xShowInfoText.count; i++) {
            ...
            textLayer.frame = CGRectMake(_originSize.x-4*size.width, CGRectGetHeight(self.frame) - _originSize.y - i*_columnWidth - i*_typeSpace - _typeSpace, 4*size.width, size.height);
...
//            itemsView.frame = CGRectMake((i * arr.count + j)*_columnWidth + i*_typeSpace+_originSize.x + _typeSpace, CGRectGetHeight(self.frame) - _originSize.y-1, _columnWidth, 0);
            itemsView.frame = CGRectMake(_originSize.x, CGRectGetHeight(self.frame) - _originSize.y - (i * arr.count + j)*_columnWidth - i*_typeSpace - _typeSpace, 0, _columnWidth);
            ...
            [UIView animateWithDuration:1 animations:^{
                ...
                if (finished) {
                    ...
                    // 文字的位置
                    textLayer.frame = CGRectMake(height + _originSize.x+2, CGRectGetHeight(self.frame) - _originSize.y - (i * arr.count + j)*_columnWidth - i*_typeSpace - _typeSpace, 2.5*size.width+4, size.height);
